                            ...
                            <div class="row">
                                ...
                                <div class="input-group mb-3">
                                    <select class="form-select" id="inputGroupSelect01" name="cari_kabupaten">
                                        ...
                                        @foreach ($isi['provinsi'] as $item)
                                            <option value="{{ $item['provinsi'] }}">
                                                {{ str_replace('_', ' ', str_replace('_', ' ', $item['provinsi'])) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    </select>
                                ...
                                <table class="table" style="width: 80%">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($isi['kabupaten'] as $item)
                                            <tr>
                                                <th scope="row">{{ $i + 1 }}</th>
                                                <td><a style="color: rgba(0, 0, 0, 0.844)"
                                                        href="/{{ $isi['link'] }}/{{ str_replace(' ', '_', $item['kabupaten']) }}">
                                                        {{ str_replace('_', ' ', $item['kabupaten']) }}</a></td>
                                            </tr>
                                            <?php $i++; ?>
                                        @endforeach
                            @endif
